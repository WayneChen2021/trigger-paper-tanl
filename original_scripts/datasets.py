...
class JointERDataset(BaseDataset):
    ...
    # dictionary from entity types given in the dataset to the natural strings to use
    natural_entity_types = None
    # dictionary from relation types given in the dataset to the natural strings to use
                ...
                relations = [
                    Relation(
                        type=self.relation_types[y['type']
                                                 ], head=entities[y['head']], tail=entities[y['tail']]
                    )
    ...
    def evaluate_example(self, example: InputExample, output_sentence: str, model=None, tokenizer=None, log_file=None) -> Counter:
        ...
        predicted_entities_no_type = set(
            [entity[1:] for entity in predicted_entities])
        ...
        gt_relations = set(relation.to_tuple()
        ...
        if self.entity_types is not None:
            for entity_type in self.entity_types.values():
                predicted = set(
                    entity for entity in predicted_entities if entity[0] == entity_type.natural)
                gt = set(
                    entity for entity in gt_entities if entity[0] == entity_type.natural)
        ...
        if self.relation_types is not None:
            for relation_type in self.relation_types.values():
                predicted = set(
                    relation for relation in predicted_relations if relation[0] == relation_type.natural)
                gt = set(
                    relation for relation in gt_relations if relation[0] == relation_type.natural)
                correct = predicted & gt
                res['predicted_relations',
                    relation_type.natural] = len(predicted)
...
class DocREDDataset(JointERDataset):
    """
    DocRED dataset (joint entity and relation extraction).
    """
    name = 'docred'
        ...
        with open(file_path, 'r') as f:
            ...
            for i, x in enumerate(data):
                ...
                for y in x['spo_details']:
                    ...
                    entity1 = Entity(
                        type=self.entity_types[entity1_type], start=entity1_start, end=entity1_end)
                    entity2 = Entity(
                        type=self.entity_types[entity2_type], start=entity2_start, end=entity2_end)
...
class ACE2005REDataset(JointERDataset):
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
            ...
            for j, l in enumerate(f):
                ...
                for i, tokens in enumerate(document['sentences']):
                    ...
                    if len(document['ner'][i]) > 0:
                        entities = [
                            Entity(
                                type=self.entity_types[entity_type], start=start-offset, end=end-offset+1)
                        ...
                        for start1, end1, start2, end2, relation_type in document['relations'][i]:
                            ...
                            [head] = [e for e in entities if e.start ==
                                      start1-offset and e.end == end1-offset+1]
                            [tail] = [e for e in entities if e.start ==
                                      start2-offset and e.end == end2-offset+1]

                            relations.append(
                                Relation(
                                    type=self.relation_types[relation_type], head=head, tail=tail)
                            )
        ...
        logging.info(
            f'Constructed {len(examples)} examples (from {num_documents} documents) for {self.name} ({split})')
        return examples
...
class NERDataset(JointERDataset):
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        logging.info(
            f"Loaded {len(raw_examples)} sentences for split {split} of {self.name}")
            ...
            for j, label in enumerate(labels + [None]):
                ...
                if (label is None and previous_label is not None) \
                        ...
                        or (label is not None and previous_label is not None and (
                            label[2:] != previous_label[2:] or label.startswith(
                                'B-') or label.startswith('S-')
                        )):
...
class ACE2005NERDataset(NERDataset):
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        logging.info(
            f"Loaded {len(examples)} sentences for split {split} of {self.name}")
        return examples
...
class ACE2005EventTriggerDataset(JointERDataset):
    ...
    def load_schema(self):
        ...
        schema_file_name = os.path.join(
            self.data_dir(), f'{self.data_name}_schema.json')
...
class MUCNERMultiPhaseTrigger(MUCEventTriggerDataset):
    ...
    default_input_format = 'muc_multiphase_ner'
        ...
        with open(file_path, 'r') as f:
            ...
            for x in data:
                ...
                input_relations = [
                    ...
                    Relation(
                        type=self.relation_types[y['type']
                                                ], head=input_entities[y['head']], tail=input_triggers[y['tail']]
                    )
                    for y in (x['first_phase']['relations'] if 'relations' in x['first_phase'] else [])
                ]
...
@register_dataset
class MUCEventMultiPhaseTrigger(MUCNERMultiPhaseTrigger):
    name = 'muc_event_multiphase_trigger'
    data_name = 'muc_event_multiphase'

    default_input_format = 'muc_multiphase_event'
    default_output_format = 'multiphase_trigger'
...
class MUCEventArgumentDataset(MUCEventTriggerDataset):
    ...
    def yield_single_document(self, docid, tokens, x, x_second = None, same_input_output_trigs=False):
        input_parts = []
        output_parts = []
        ...
        input_parts = self.handle_single_trig(x, tokens, docid)

        if not (same_input_output_trigs and self.is_eval):
            if len(x_second['triggers']) <= 1:
                output_parts.append(self.handle_single_trig(x_second, tokens, docid))
            else:
    ...
    def load_data_single_split(self, split: str, seed: int = None, same_input_output_trigs=False) -> List[InputExample]:
        """
        Load data for a single split (train, dev, or test).
        """
        examples = []
        ...
        with open(file_path, 'r') as f:
            ...
            for i, x in enumerate(data):
                if not 'second_phase' in x:
                    ...
                    examples += self.yield_single_document(x['id'], x['tokens'], x['first_phase'], x['second_phase'], same_input_output_trigs)
...
@register_dataset
class WikiEventsArgumentDataset(MUCEventArgumentDataset):
...
@register_dataset
class MUCEventMultiPhaseArgument(MUCNERMultiPhaseArgument):
    name = 'muc_event_multiphase_argument'
    data_name = 'muc_event_multiphase'
...
@register_dataset
class WikiEventsDataset(WikiEventsArgumentDataset):
    name = 'wikievents'
...
class CoNLL12CorefDataset(BaseDataset):
        ...
        with open(file_path, 'r') as f:
            for i, l in enumerate(f):
                ...
                for raw_group in raw_document['annotations']['coreference']['groups']:
                    ...
                    for raw_mention in raw_group['mentions']:
                        ...
                        start = bisect.bisect_left(
                            tokens_start_char, raw_mention['start'])
                        end = bisect.bisect_left(
                            tokens_end_char, raw_mention['end']) + 1
                ...
                while pos < len(tokens):
                    ...
                    for group in groups:
                        mentions = [
                            Entity(start=mention.start-pos,
                                   end=mention.end-pos, type=mention.type)
                ...
                self.documents[document_id] = CorefDocument(
                    ...
                    groups=groups,
                    chunks=chunks,
                    chunk_centers=[example.offset +
                                   len(example.tokens) // 2 for example in chunks]
                )
    ...
    def evaluate_dataset(self, data_args, model, device, batch_size=8, macro=False, by_relation_type=False) \
        ...
        for example, output_sentence in self.generate_output_sentences(data_args, model, device, batch_size):
            ...
            data = [tuple(tuple(y + offset for y in x) for x in z)
                    for z in data if z[1] is not None]
            ...
            if len(documents_to_chunk_data[document_id]) == len(self.documents[document_id].chunks):
                ...
                predictions[document_id] = self.get_document_predictions(
                    documents_to_chunk_data[document_id])
...
class RelationClassificationDataset(JointERDataset):
    ...
    def nll_inference(self, example: InputExample, relation_types: List[RelationType], model=None,
        ...
        for relation_type in relation_types:
            ...
            formatted_output = [
                self.output_format.format_output(candidate_example)]
...
class FewRelFull(RelationClassificationDataset):
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        examples = [example for x in examples_by_type.values()
                    for example in x]
        return examples
...
class FewRelEpisodic(FewRelFull):
    ...
    # the relation types involved in this episode (there is self.num_ways of them)
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        Load data for a single split (train, dev, or test).
        """
        examples_by_type = self.load_data_by_relation_type(
            split='dev')   # we use the dev set for episodic experiments
        ...
        for type_id in self.target_relation_types:
            sampled_examples = random.sample(
                examples_by_type[type_id], num_shots + num_queries)
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        with open(file_path, 'r') as f:
            ...
            for idx, obj in enumerate(data):
                words = obj['token']
                head_start, head_end, head_type = obj['subj_start'], obj['subj_end'] + \
                    1, obj['subj_type']
                tail_start, tail_end, tail_type = obj['obj_start'], obj['obj_end'] + \
                    1, obj['obj_type']
                relation = obj['relation']

                if head_type not in self.entity_types:
                    self.entity_types[head_type] = EntityType(
                        short=head_type, natural=self.to_natural(head_type))
                if tail_type not in self.entity_types:
                    self.entity_types[tail_type] = EntityType(
                        short=tail_type, natural=self.to_natural(tail_type))
                ...
                if relation not in self.relation_types:
                    self.relation_types[relation] = RelationType(
                        short=relation, natural=self.to_natural(relation))

                relations = [
                    Relation(
                        type=self.relation_types[relation], head=head_entity, tail=tail_entity)
                ]
...
class MultiWoz(BaseDataset):
    ...
    def truncate_first_n_tokens(self, examples, max_seq_length, delimiter='▁'):
        ...
        for x in examples:
            ...
            if len(tokens) > max_seq_length:
                x = ''.join(tokens[-1 * max_seq_length + 1:]
                            ).replace(delimiter, ' ')
                assert self.tokenizer.tokenize(
                    ''.join(tokens).replace(delimiter, ' ')) == tokens
            output.append(x)
        return output
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        examples = []
        file_path = os.path.join(
            self.data_dir(), f'multi_woz_2.1_{split}_5_domain.json')

        with open(file_path, 'r') as f:
            data = json.load(f)
            num_examples = len(data["examples"])
            logging.info(
                f"Loaded {num_examples} sentences for split {split} of {self.name}")
