...
class InputExample:
    ...
    # ADD ON
    output_entities: List[Entity] = None
...
class CorefDocument:
    ...
    id: str                     # unique id in the dataset
    tokens: List[str]           # list of tokens (words)
    # list of chunks for this document (the offset is an attribute of the InputExample)
    chunks: List[InputExample]
    # list of the centers of the chunks (useful to find the chunk with largest context)
...
class InputFeatures:
    ...
    use_non_args_masked: Optional[float] = None
