...
def span_scorer(span1, span2, span_mode="geometric_mean"):
    ...
    if span_mode == "absolute":
        ...
        intersection = max(
            0, min(span1[1], span2[1]) - max(span1[0], span2[0]))
        return 1 - (
        ...
        if verbosity == 3 or verbosity == 4:
            ...
            for key in ["total"] + role_names:
                result_string += key + ": Precision : {1:.4f}, Recall : {2:.4f}, F1 : {0:.4f}\n".format(
                    ...
                    self.stats[key]["r"])
                info_json[key] = {
                    "precision": self.stats[key]["p"],
                    "recall": self.stats[key]["r"],
                    "f1": self.stats[key]["f1"]
                }

            global muc_error_file
            with open(muc_error_file, "a") as ea_file:
    ...
    def combine(result1, result2):
        ...
        for key in result.stats.keys():
            for stat in ["num", "p_den", "r_den"]:
                result.stats[key][stat] = result1.stats[key][stat] + \
                    result2.stats[key][stat]
        ...
        for role_name in result.missing_roles:
            result.missing_roles[role_name] = result1.missing_roles[role_name] + \
                result2.missing_roles[role_name]
            result.incorrect_roles[role_name] = result1.incorrect_roles[role_name] + \
                result2.incorrect_roles[role_name]
        return result
    ...
    def compute(self, template_matching, docid):
        ...
        for template, role_name, corefs in self.missing_rfs:
            if str(template) in hand_mprfs:
                if role_name in hand_mprfs[str(template)]:
                    hand_mprfs[str(
                        template)][role_name] += [mention for mention in corefs]
                else:
                    hand_mprfs[str(template)][role_name] = [
                        mention for mention in corefs]
            else:
        ...
        for ind, trans in enumerate(self.transformations):
            if trans == "\n":
                ...
                mis = (trans[3], None, [
                       "Introduce_Missing_Role_Filler"], trans[1])
                remove_transformations.append(mis)
            elif trans[2] == ["Alter_Span", "Alter_Role"]:
                altered_transformations.insert(temp_pos, trans)
                mis = (trans[3], None, [
                       "Introduce_Missing_Role_Filler"], trans[4])
                remove_transformations.append(mis)
            else:
                altered_transformations.append(trans)
        ...
        for tidx, trans in enumerate(self.transformations):
            if trans == "\n":
                ...
                if pred_templates[pair_count] == None:
                    pred_templates[pair_count] = copy.deepcopy(
                        org_pred_templates[pair_count])
                    ...
                    if trans[1] in pred_templates[pair_count][trans[3]]:
                        ...
                        try:
                            if trans[1] in hand_mprfs[str(org_pred_templates[pair_count])][trans[3]]:
                                pred_templates[pair_count][trans[3]].append(
                                    trans[1])
                        except:
                            continue
            elif trans[2] == ["Alter_Span", "Alter_Role"]:
                ...
                if pred_templates[pair_count] != None:
                    ...
                    if pred_templates[temp_idx] == None:
                        pred_templates[temp_idx] = copy.deepcopy(
                            org_pred_templates[temp_idx])
                        ...
                        if (trans[1] in pred_templates[temp_idx][trans[0]]):
                            ...
                            try:
                                if trans[1] in hand_mprfs[str(org_pred_templates[temp_idx])][trans[0]] and trans[
                                    ...
                                    pred_templates[temp_idx][trans[0]].append(
                                        trans[1])
                            except:
                                continue
                        ...
                        if trans[1] in pred_templates[temp_idx][trans[3]]:
                            ...
                            try:
                                if trans[1] in hand_mprfs[str(org_pred_templates[temp_idx])][trans[3]] and trans[
                                        1] not in hand_sprfs[str(org_pred_templates[temp_idx])]:
                                    pred_templates[temp_idx][trans[3]].append(
                                        trans[1])
                            except:
                                continue
                ...
                for role_key in trans[3]:
                    ...
                    if trans[3][role_key] == []:
                        ...
                        for coref in trans[3][role_key]:
                            pred_templates[pair_count][role_key].append(
                                coref[0])

            else:
                raise Exception("Incorrect transformation type")

        proc_pred_templates = [temp for temp in pred_templates if not (
            temp == None or temp == "Removed")]
        proc_gold_templates = [temp for temp in gold_templates if not (
            temp == None or temp == "Removed")]
        self.transformed_data = [
            (docid, (proc_pred_templates, proc_gold_templates))]
...
def analyze(
            ...
            if len(predicted_mentions) == 0:
                ...
                for i in range(len(gold_mentions)):
                    ...
                    for matching in mention_matches(
                        ...
                        yield [
                            (predicted_mentions[0], gold_mentions[i],
                             best_score, best_gold_mention)
                        ] + matching
        ...
        for template_pair in template_matching:
            ...
            if template_pair[0] is None and scoring_mode in [
                ...
                if mode in ["MUC_Errors", "Errors"]:
                    ...
                    pairwise_result.transformations.append(
                        ("", None, ["Introduce_Missing_Template"], template_pair[1]))
                ...
                if mode in ["MUC_Errors", "Errors"]:
                    ...
                    pairwise_result.transformations.append(
                        ("", template_pair[0], ["Remove_Spurious_Template"], None))
                ...
                for role_name in role_names:
                            ...
                            for mention_pair in mention_matching:
                                if mention_pair[0] is None:
                                    ...
                                    if (
                                        ...
                                        diff_decide(
                                            matching_result, mention_pair[0], mention_pair[3])
                                    if (
                                            mode in ["MUC_Errors", "Errors"]
                    ...
                    pairwise_result = Result.combine(
                        pairwise_result, rolewise_result)
    ...
    def handle_spurious_rfs(best_result, best_matching):
        ...
        for ridx, sprf in enumerate(best_result.spurious_rfs):
            ...
            transform_idx = best_result.transformations.index(
                ("", "", ["Remove_Spurious_Role_Filler"], ""))
                                    ...
                                    if min_score == 0:
                                        ...
                                        best_result.transformations[transform_idx] = (
                                            pred_role_name, pred_mention, [
                                                "Alter_Span", "Alter_Role"], role_name,
                                            best_gold_mention)
                                        best_result.errors[
        ...
        best_result.spurious_rfs = [
            sprf for sprf in best_result.spurious_rfs if sprf != "Removed"]
...
def from_file(input_file, mode):
    ...
    for docid, example in inp_dict.items():
        ...
        doc_tokens = normalize_string(
            example["doctext"].replace(" ##", "")).split(" ")
...
def main():
    ...
    def add_script_args(parser):
        ...
        parser.add_argument("--muc_errors", type=str, required=True)
        return parser
    ...
    global muc_error_file
    muc_error_file = args.muc_errors
    ...
    for docid, pair in data:
        ...
        for idx, template_pair in enumerate(matching):
            output_file.write("\n\n\t---\n\n")
            output_file.write("Template Pair " +
                              str(idx + 1) + ": \nMatching:")
    ...
    if analyze_transformed:
        ...
        for docid, pair in tqdm(total_result_before.transformed_data, desc="Analyzing Transformed Data: "):
            ...
            result, matching = analyze(
                docid, *pair, mode, scoring_mode, verbose)
